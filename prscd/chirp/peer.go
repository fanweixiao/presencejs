...
type Peer struct {
	...
	Cid string
	// Channel descrbes the channel which this peer joined.
...
func (p *Peer) NotifyBack(sig *Signalling) {
...
func (p *Peer) Leave(channelName string) {
	...
	// remove pper from channel's peer list
...
func (p *Peer) BroadcastToChannel(sig *Signalling) {
	sig.Cid = p.Cid
...
func (p *Peer) HandleSignal(r io.Reader) error {
	...
	if sig.Type == SigControl {
		// handle the Control Singalling
		switch sig.OpCode {
		case OpChannelJoin: // `channel_join` signalling
			// join channel
			p.Join(sig.Channel)
		case OpState: // `peer_state` signalling
		...
		case OpPeerOffline: // `peer_offline` signalling
			p.Leave(sig.Channel)
		case OpPeerOnline: // `peer_online` signalling
			p.BroadcastToChannel(sig)
		default:
			log.Error("Unknown control opcode: %d", sig.OpCode)
		}
	} else if sig.Type == SigData {
