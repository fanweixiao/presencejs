...
// ConnectToYoMo connect this node to who geo-disributed network which built by yomo.
func (n *node) ConnectToYoMo(sndr yomo.Source, rcvr yomo.StreamFunction) error {
	...
	rcvr.SetObserveDataTags(0x20)

	// handle data from yomo network, and dispatch to the same channel on this node.
	rcvr.SetHandler(func(msg []byte) (frame.Tag, []byte) {
		var sig *Signalling
		err := msgpack.Unmarshal(msg, &sig)
		if err != nil {
			log.Error("Read from YoMo error: %v, msg=%# x, string(msg)=%s", err, msg, msg)
		}
		...
		if channel != nil {
			...
			log.Debug("[\u21CA]\t dispathched to %s", sig.Cid)
		} else {
			log.Debug("[\u21CA]\t dispathch to channel failed cause of not exist: %s", sig.Cid)
		}
...
func (n *node) BroadcastToYoMo(sig *Signalling) {
	...
	err = n.sndr.WriteWithTag(0x20, buf)
	if err != nil {
		log.Error("broadcast to yomo: %+v", err)
	}
}
